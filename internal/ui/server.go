...
// Package ui hosts the FreeCCR-go web UI: the embedded SPA plus the JSON/binary
// API (load a roll, sample B/W points, convert + adjust with a live preview,
// sync across the roll, export). Exposed as a single http.Handler so both the
// freeccrd HTTP server and the Wails desktop app share one implementation.
package ui
...
// Server holds the loaded-roll session behind the UI's HTTP handlers.
type Server struct {
	sess *session.Session
}

// NewServer creates a UI server with an empty session.
func NewServer() *Server {
	return &Server{sess: session.New()}
}

// LoadDir loads a roll directory into the session at startup (optional).
func (s *Server) LoadDir(dir string) error {
	_, err := s.sess.Load(dir, nil)
	return err
}

// Handler returns the http.Handler serving the SPA (/) and API (/api/*).
...
func (s *Server) handleIndex(w http.ResponseWriter, r *http.Request) {
	if os.Getenv("FREECCR_DEBUG") != "" {
		fmt.Fprintln(os.Stderr, "ui: serving", r.URL.Path)
	}
	if r.URL.Path != "/" {
		http.NotFound(w, r)
		return
